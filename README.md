...
> 학원에서 C 13일차까지 배운 뒤 시작한 C++ 강의 첫날 실습 코드 모음입니다. Visual Studio에서 작성/실행했으며, 각 예제마다 개념 설명과 실제 실행 결과를 함께 정리했습니다.

## 목차

- [1-1. cout으로 화면에 출력하기](#1-1-cout으로-화면에-출력하기)
- [1-2. cin으로 두 숫자 입력받아 덧셈하기](#1-2-cin으로-두-숫자-입력받아-덧셈하기)
- [1-3. 두 수 사이의 정수 합 구하기](#1-3-두-수-사이의-정수-합-구하기)
- [1-4. 여러 개의 char 배열로 정보 입력받기](#1-4-여러-개의-char-배열로-정보-입력받기)
- [1-5. 함수 오버로딩 (다중정의)](#1-5-함수-오버로딩-다중정의)
- [1-6. 디폴트 매개변수 2개 함께 쓰기](#1-6-디폴트-매개변수-2개-함께-쓰기)
- [1-7. inline 함수](#1-7-inline-함수)
- [1-8. namespace(이름공간) 기초](#1-8-namespace이름공간-기초)
- [1-9. 중첩 namespace와 별칭(alias)](#1-9-중첩-namespace와-별칭alias)
- [1-10. bool 자료형, while(true)+break, sizeof](#1-10-bool-자료형-whiletruebreak-sizeof)
- [1-11. bool을 반환하는 함수로 양수/음수 판별하기](#1-11-bool을-반환하는-함수로-양수음수-판별하기)
- [1-12. 참조자(reference) — 변수에 별명 붙이기](#1-12-참조자reference-변수에-별명-붙이기)
- [1-13. 배열 원소에 참조자 걸기](#1-13-배열-원소에-참조자-걸기)
- [1-14. 참조자와 포인터를 함께 쓰기](#1-14-참조자와-포인터를-함께-쓰기)
...
[디폴트 매개변수란] 함수를 선언할 때 매개변수에 미리 기본값을 지정해두는 기능입니다(int num = 7). 함수를 호출할 때 그 자리의 인자를 생략하면, 지정해둔 기본값이 자동으로 사용됩니다.
...
[오버로딩과의 관계] 매개변수 개수가 다른 여러 함수(Adder(a), Adder(a,b))를 따로 만드는 대신, 디폴트 매개변수 하나로 "인자를 몇 개 주든 알아서 동작하는" 함수 하나를 만들 수 있어 오버로딩의 부담을 줄여줍니다. (opp6 파일에서 int Adder(int num1=1, int num2=2) 형태로 두 개를 동시에 실습했습니다.)
...
#### 🔁 변형 예제: using namespace로 소속 생략하기
...
### 1-10. bool 자료형, while(true)+break, sizeof
...
### 1-11. bool을 반환하는 함수로 양수/음수 판별하기
...
VAL: 3047
PTR이 가리키는 값: 3047
num1의 주소: 0x7ffdacfdc83c
ptr에 저장된 주소값: 0x7ffdacfdc83c
